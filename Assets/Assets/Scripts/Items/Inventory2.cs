...
public class Inventory2 : MonoBehaviour{
	...
	public void Start () { //run this everytime the inventory is updated
		BaseItem inventoryItem = GetComponent<BaseItem>(); //gets the item that is attached to the player
		...
		mGold.text = "Gold: " + GetComponent<PlayerInfo> ().gold.ToString () + "G"; //gets the gold from the player and shows it in the inventory
	...
	void Update () {
		if (Input.GetKeyDown(KeyCode.I)) { //checks if the user presses I. If so, toggles the inventory screen.
			if (_InventoryActive == false) {
				Start (); //runs the start again so the values are updated
